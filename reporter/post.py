...
def main():
    ...
    if args.pr.lower() != 'none' and args.github_token.lower() != 'none':
        url = DEFAULT_URL.format(OWNER=os.environ['REPOSITORY_OWNER'],
...
def post_comment(url: str, token: str, file_path: Path):
    ...
    comment = next((comment for comment in comments if
                    comment['user']['login'] == 'github-actions[bot]' and 'Robot Framework Report' in comment['body']),
                   None)
    if comment:
